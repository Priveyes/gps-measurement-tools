 ...
 */
public class MainActivity extends AppCompatActivity implements OnConnectionFailedListener, ConnectionCallbacks, GroundTruthModeSwitcher {
	...
	protected void onResume() {
		super.onResume();
		LocalBroadcastManager.getInstance(this).registerReceiver(mBroadcastReceiver, new IntentFilter(DetectedActivitiesIntentReceiver.AR_RESULT_BROADCAST_ACTION));
	}
	...
	private synchronized void buildGoogleApiClient() {
		mGoogleApiClient = new GoogleApiClient.Builder(this).enableAutoManage(this, this).addConnectionCallbacks(this).addOnConnectionFailedListener(this).addApi(ActivityRecognition.API).build();
	}
	...
	public void onConnected(Bundle connectionHint) {
		...
		ActivityRecognition.ActivityRecognitionApi.requestActivityUpdates(mGoogleApiClient, 0, createActivityDetectionPendingIntent());
	}
	...
	private void setupFragments() {
		...
		mRealTimePositionVelocityCalculator.setResidualPlotMode(RealTimePositionVelocityCalculator.RESIDUAL_MODE_DISABLED, null /* fixedGroundTruth */);

		mFileLogger = new FileLogger(getApplicationContext());
		mAgnssUiLogger = new AgnssUiLogger();
		mGnssContainer = new GnssContainer(getApplicationContext(), mUiLogger, mFileLogger, mRealTimePositionVelocityCalculator, mAgnssUiLogger);
	...
	private void requestPermissionAndSetupFragments(final AppCompatActivity activity) {
	...
	private void setGroundTruthModeOnResult(ActivityRecognitionResult result) {
		if (result != null) {
			...
			if (detectedActivityType == DetectedActivity.STILL || detectedActivityType == DetectedActivity.TILTING) {
				mRealTimePositionVelocityCalculator.setResidualPlotMode(RealTimePositionVelocityCalculator.RESIDUAL_MODE_STILL, null);
			} else {
				mRealTimePositionVelocityCalculator.setResidualPlotMode(RealTimePositionVelocityCalculator.RESIDUAL_MODE_MOVING, null);
			}
		}
	}
}
