...
public class MapFragment extends Fragment implements OnMapReadyCallback {
	...
	private static final SimpleDateFormat DATE_SDF = new SimpleDateFormat("HH:mm:ss", Locale.getDefault());

	// UI members
	private GoogleMap mMap; // Might be null if Google Play services APK is not available.
	private MapView mMapView;
	private final Set<Object> mSetOfFeatures = new HashSet<>();
	...
	public void updateMapViewWithPostions(final double latDegRaw, final double lngDegRaw, final double latDegDevice, final double lngDegDevice, final long timeMillis) {
		/*AppCompatActivity*/
		...
		activity.runOnUiThread(() -> {
			...
			if (mLastLocationMarkerRaw == null && mLastLocationMarkerDevice == null) {
				if (mMap != null) {
					mLastLocationMarkerDevice = mMap.addMarker(new MarkerOptions().position(latLngDevice).title(getResources().getString(R.string.title_device)).icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE)));
					mLastLocationMarkerDevice.showInfoWindow();

					mLastLocationMarkerRaw = mMap.addMarker(new MarkerOptions().position(latLngRaw).title(getResources().getString(R.string.title_wls)).icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN)));
