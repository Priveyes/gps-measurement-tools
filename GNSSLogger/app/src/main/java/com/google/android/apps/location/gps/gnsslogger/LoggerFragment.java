...
public class LoggerFragment extends Fragment implements TimerListener {
	...
	private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			TimerValues countdown = new TimerValues(intent.getLongExtra(TimerService.EXTRA_KEY_UPDATE_REMAINING, 0L));
	...
	public void onResume() {
		super.onResume();
		LocalBroadcastManager.getInstance(getActivity()).registerReceiver(mBroadcastReceiver, new IntentFilter(TimerService.TIMER_ACTION));
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		mStartLog.setOnClickListener(view -> {
		...
		mSendFile.setOnClickListener(view -> stopAndSend());

		mTimer.setOnClickListener(view -> launchTimerDialog());
	...
	void displayTimer(TimerValues values, boolean countdownStyle) {
		...
		mTimerDisplay.setText(String.format("%s: %s", getResources().getString(R.string.timer_display), content));
	}
