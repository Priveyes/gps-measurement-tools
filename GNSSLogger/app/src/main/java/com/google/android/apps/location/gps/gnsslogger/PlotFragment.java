...
public class PlotFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		OnItemSelectedListener spinnerOnSelectedListener = new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
				...
				XYMultipleSeriesRenderer renderer = mDataSetManager.getRenderer(mCurrentTab, spinner.getSelectedItemPosition());
				XYMultipleSeriesDataset dataSet = mDataSetManager.getDataSet(mCurrentTab, spinner.getSelectedItemPosition());
	...
	protected void updateCnoTab(GnssMeasurementsEvent event) {
		...
		if (measurements.size() >= NUMBER_OF_STRONGEST_SATELLITES) {
			mAverageCn0 = (mAverageCn0 * mMeasurementCount + (measurements.get(0).getCn0DbHz() + measurements.get(1).getCn0DbHz() + measurements.get(2).getCn0DbHz() + measurements.get(3).getCn0DbHz()) / NUMBER_OF_STRONGEST_SATELLITES) / (++mMeasurementCount);
			currentAverage = (measurements.get(0).getCn0DbHz() + measurements.get(1).getCn0DbHz() + measurements.get(2).getCn0DbHz() + measurements.get(3).getCn0DbHz()) / NUMBER_OF_STRONGEST_SATELLITES;
		}
		...
		for (int i = 0; i < NUMBER_OF_STRONGEST_SATELLITES && i < measurements.size(); i++) {
			...
			builder.append(mDataSetManager.getConstellationPrefix(measurements.get(i).getConstellationType()) + measurements.get(i).getSvid() + ": " + sDataFormat.format(measurements.get(i).getCn0DbHz()) + "\n");
			int end = builder.length();
			builder.setSpan(new ForegroundColorSpan(mColorMap.getColor(measurements.get(i).getSvid(), measurements.get(i).getConstellationType())), start, end, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
		}
		...
		for (GnssMeasurement measurement : measurements) {
			...
			if (constellationType != GnssStatus.CONSTELLATION_UNKNOWN) {
				mDataSetManager.addValue(CN0_TAB, constellationType, svID, mLastTimeReceivedSeconds, measurement.getCn0DbHz());
			}
		}
	...
	protected void updatePseudorangeResidualTab(double[] residuals, double timeInSeconds) {
		...
		for (int i = 1; i <= GpsNavigationMessageStore.MAX_NUMBER_OF_SATELLITES; i++) {
			if (!Double.isNaN(residuals[i - 1])) {
				mDataSetManager.addValue(PR_RESIDUAL_TAB, GnssStatus.CONSTELLATION_GPS, i, timeSinceLastMeasurement, residuals[i - 1]);
			}
		}
	...
	private List<GnssMeasurement> sortByCarrierToNoiseRatio(List<GnssMeasurement> measurements) {
		Collections.sort(measurements, (o1, o2) -> Double.compare(o2.getCn0DbHz(), o1.getCn0DbHz()));
		return measurements;
	}
	...
	private static class ColorMap {
		 ...
		 */
		private static final String[] CONTRASTING_COLORS = {"#222222", "#F3C300", "#875692", "#F38400", "#A1CAF1", "#BE0032", "#C2B280", "#848482", "#008856", "#E68FAC", "#0067A5", "#F99379", "#604E97", "#F6A600", "#B3446C", "#DCD300", "#882D17", "#8DB600", "#654522", "#E25822", "#2B3D26"};
	...
	private static class DataSetManager {
		...
		public DataSetManager(int numberOfTabs, int numberOfConstellations, Context context, ColorMap colorMap) {
			...
			for (int i = 0; i < numberOfTabs; i++) {
				...
				for (int k = 0; k <= numberOfConstellations; k++) {
					mSatelliteIndex[i].add(new ArrayMap<>());
					mSatelliteConstellationIndex[i].add(new ArrayMap<>());
		...
		private String getConstellationPrefix(int constellationType) {
			if (constellationType <= GnssStatus.CONSTELLATION_UNKNOWN || constellationType > NUMBER_OF_CONSTELLATIONS) {
				return "";
			}
		 ...
		 */
		private void addValue(int tab, int constellationType, int svID, double timeInSeconds, double value) {
			...
			if (hasSeen(constellationType, svID, tab)) {
				...
				// data
				dataSetAll.getSeriesAt(mSatelliteIndex[tab].get(constellationType).get(svID)).add(timeInSeconds, value);
				mDataSetList[tab].get(constellationType).getSeriesAt(mSatelliteConstellationIndex[tab].get(constellationType).get(svID)).add(timeInSeconds, value);
			} else {
				...
				mSatelliteConstellationIndex[tab].get(constellationType).put(svID, mDataSetList[tab].get(constellationType).getSeriesCount());
		...
		private void setUpRenderer(XYMultipleSeriesRenderer renderer, int tabNumber) {
			...
			renderer.setChartTitle(mContext.getResources().getStringArray(R.array.plot_titles)[tabNumber]);
