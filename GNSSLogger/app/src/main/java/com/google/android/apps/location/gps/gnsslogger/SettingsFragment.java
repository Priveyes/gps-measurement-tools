...
public class SettingsFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		registerLocation.setOnCheckedChangeListener((buttonView, isChecked) -> {
		...
		registerMeasurements.setOnCheckedChangeListener((buttonView, isChecked) -> {
		...
		registerNavigation.setOnCheckedChangeListener((buttonView, isChecked) -> {
		...
		registerGpsStatus.setOnCheckedChangeListener((buttonView, isChecked) -> {
		...
		registerNmea.setOnCheckedChangeListener((buttonView, isChecked) -> {
		...
		residualPlotSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {

                LayoutInflater inflater1 = (LayoutInflater) getActivity().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                View layout = inflater1.inflate(R.layout.pop_up_window, (ViewGroup) getActivity().findViewById(R.id.pop));
                ...
                popupWindow.showAtLocation(view.findViewById(R.id.setting_root), Gravity.CENTER, 0, 0);
                View container1 = (View) popupWindow.getContentView().getParent();
                WindowManager wm = (WindowManager) getActivity().getSystemService(Context.WINDOW_SERVICE);
                WindowManager.LayoutParams params = (WindowManager.LayoutParams) container1.getLayoutParams();
                ...
                popupWindow.setOnDismissListener(() -> {
                    if (mResidualSetting == RealTimePositionVelocityCalculator.RESIDUAL_MODE_DISABLED) {
                        ...
                        mRealTimePositionVelocityCalculator.setResidualPlotMode(mResidualSetting, mFixedReferenceLocation);
                ...
                buttonOk.setOnClickListener(v -> {
                    double longitudeDegrees = longitudeInput.getText().toString().equals("") ? Double.NaN : Double.parseDouble(longitudeInput.getText().toString());
                    double latitudeDegrees = latitudeInput.getText().toString().equals("") ? Double.NaN : Double.parseDouble(latitudeInput.getText().toString());
                    double altitudeMeters = altitudeInput.getText().toString().equals("") ? Double.NaN : Double.parseDouble(altitudeInput.getText().toString());
                ...
                mRealTimePositionVelocityCalculator.setResidualPlotMode(RealTimePositionVelocityCalculator.RESIDUAL_MODE_DISABLED, mFixedReferenceLocation);
                turnOnResidual.setText("Switch is OFF");
            }
        });
		...
		help.setOnClickListener(view12 -> helpDialog.show());

		Button exit = (Button) view.findViewById(R.id.exit);
		exit.setOnClickListener(view1 -> getActivity().finishAffinity());
